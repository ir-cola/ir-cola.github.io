...
- モーダルに「権利表記・注記」欄を追加（`workData[id].notice = {ja,en}` がある作品のみ、説明文の下に `.work-detail-notice` として表示。左に赤ライン・薄背景の枠つき小文字、`white-space: pre-line`。notice 無しの作品は `display:none`）。東方春三校に二次創作表記を設定＝①『東方Project』（原作：上海アリス幻樂団／ZUN氏）の二次創作である旨 ②上海アリス幻樂団および原作関係者が制作・監修したものではない非公式作品である旨 ③学内課題の非営利・個人制作で一般配布・販売はしていない旨 ④素材はニコニ・コモンズ等で配布されているフリーの二次創作素材を各配布元の利用条件の範囲で使用（本人確認済み）、原作のゲームデータそのものは不使用 ⑤『東方Project』の権利は上海アリス幻樂団に帰属。あわせて desc 冒頭（workData・一覧カード data-ja/data-en・表示テキスト）を「『東方Project』の二次創作として制作した、非営利・個人制作のシューティング」に書き換え、desc の「フリー素材を集めて」→「ニコニ・コモンズなどで配布されているフリーの二次創作素材を利用条件の範囲で集め」に修正。※公式ガイドライン <https://touhou-project.news/guideline/> は当環境の egress proxy でブロックされ直接参照できなかったため、文面は検索で得られた要点（二次創作である旨の明示／権利は上海アリス幻樂団に帰属／個人・非営利の範囲／企業の営利利用は個別連絡）に基づく。原文の確認は本人が行う

...
const workData = {
    ...
    'touhou': {
        ...
        env: 'Unity',
        desc: {
            ja: '『東方Project』（原作：上海アリス幻樂団）の二次創作として制作した、非営利・個人制作のシューティングゲーム。プログラマーとして本格的に取り組んだ、記念すべき1作目でもある。元々原作のファンだったことから、ジャンルは迷わずシューティングを選んだ。プレイヤーへの自機狙い、敵の挙動パターン、残機システムなど、シューティングに必要な基本要素を一通り実装。Unityのビジュアルスクリプティングで構築したため作業効率は決して高くなく、調べながらの試行錯誤が続いたが、当時の自分なりに「できることを全部詰め込もう」と取り組んだ意欲が評価され、初出展のコンテストで賞を頂くことができた。\n\n工夫した点は、画面の比率・解像度を東方Project原作に合わせて4:3で構成したこと、ニコニ・コモンズなどで配布されているフリーの二次創作素材を利用条件の範囲で集め、見た目の雰囲気を原作寄りに統一したこと、自機の移動速度を調整して操作感まで原作に寄せたこと。プログラマーとしてのスタート地点であり、「ゲームを最後まで作りきる」体験を初めて得た作品。',
            en: 'A non-commercial, solo-made shoot \'em up created as a fan work of Touhou Project (original work by Team Shanghai Alice). It is also my very first complete project as a programmer. Being a fan of the original series, I naturally settled on a shoot \'em up. The build covers the genre fundamentals — aimed shots toward the player, enemy behavior patterns, and a lives system — all implemented through Unity\'s Visual Scripting. The workflow leaned heavily on visual scripting and constant lookups, so iteration was slow, but the determination to "cram in everything I could do at the time" came through, and the project earned an award at the very first contest I ever submitted to.\n\nThe areas I focused on were authenticity to the source material. The screen aspect ratio and resolution were tuned to Touhou Project\'s 4:3 layout, freely distributed fan-made assets from sources like Niconi Commons were curated — within their stated terms of use — to align the look with the original, and even the player ship\'s movement speed was adjusted so the feel matched the source. This was my starting line as a programmer — and the first time I experienced "finishing a game all the way through."'
        },
        notice: {
            ja: '【二次創作についての表記】\n本作は『東方Project』（原作：上海アリス幻樂団／ZUN氏）の二次創作作品です。上海アリス幻樂団および原作関係者が制作・監修したものではない非公式作品であり、学内課題として制作した非営利の個人制作物です。一般への配布・販売は行っていません。ゲーム内の素材は、ニコニ・コモンズをはじめとする配布フリーの二次創作素材を各配布元の利用条件の範囲で使用しており、原作のゲームデータそのものは使用していません。『東方Project』に関する権利は上海アリス幻樂団に帰属します。',
            en: 'Fan work notice\nThis is an unofficial fan work based on Touhou Project (original work by Team Shanghai Alice / ZUN). It was neither produced nor supervised by Team Shanghai Alice, and was made as a non-commercial solo school assignment; it has never been distributed or sold to the public. All in-game assets are freely distributed fan-made materials — from Niconi Commons and similar sources — used within the terms set by each distributor; no data from the original games is used. All rights to Touhou Project belong to Team Shanghai Alice.'
        },
        video: { type: 'placeholder', src: '' },
...
function openModal(workId) {
    ...
    // 権利表記・注記（二次創作作品の明示など。notice がある作品のみ表示）
    const noticeEl = modal.querySelector('.work-detail-notice');
    const notice = data.notice ? (typeof data.notice === 'object' ? data.notice[lang] : data.notice) : '';
